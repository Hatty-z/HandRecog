...
data_path=r'D:\Projects\HandGestureRecognition\leapGestRecog'
...
def get_bounding_box(img):
    ...
    for cnt in contours:
        ...
        x_min = max(0, (x_center - w / 2) / width)
        y_min = max(0, (y_center - h / 2) / height)
        x_max = min(1, (x_center + w / 2) / width)
        y_max = min(1, (y_center + h / 2) / height)
    ...
    for gesture in os.listdir(group_path):
        ...
        for img_file in os.listdir(gesture_path):
            if img_file.endswith('.png'):
                ...
                x_min, y_min, x_max, y_max = get_bounding_box(img) 
